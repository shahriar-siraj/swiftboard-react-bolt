...
import {Link as LinkIcon, ExternalLink, Trash2, Plus, Globe, Github, Server, Bookmark, Pencil} from 'lucide-react';
...
interface LinkListProps {
  ...
  onEditLink: (id: string, name: string, url: string, type: ProjectLink['type']) => Promise<void>;
}
...
export default function LinkList({ links, onDeleteLink, onAddLink, onEditLink }: LinkListProps) {
  const [isAddingLink, setIsAddingLink] = useState(false);
  const [editingLinkId, setEditingLinkId] = useState<string | null>(null);
  const [editName, setEditName] = useState("");
  const [editUrl, setEditUrl] = useState("");
  const [editType, setEditType] = useState<ProjectLink['type'] | 'other'>('other');
  ...
  const handleUpdate = async (e: React.FormEvent, linkId: string) => {
    e.preventDefault();
    await onEditLink(linkId, editName, editUrl, editType);
    setEditingLinkId(null);
  };
                ...
                <div className="space-y-3">
                  {group.links.map(link => (
                      ...
                      {editingLinkId === link.id ? (
                          <form onSubmit={(e) => {handleUpdate(e, link.id)}} className="mt-4 space-y-4">
                            <div>
                              <input
                                  type="text"
                                  name="linkName"
                            ...
                            <div>
                              <select
                                  ...
                                  value={editType}
                                  onChange={(e) => setEditType(e.target.value as ProjectLink['type'])}
                                  required
                              >
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <a
                                ...
                                className="mt-1 inline-flex items-center text-primary-500 hover:text-primary-600 text-sm break-all"
                            >
                              {link.url}
